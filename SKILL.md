...
3. **Check and install dependencies** (first run only):
   ...
   ```bash
   /usr/bin/python3 -c "import cv2" 2>/dev/null || python3 -c "import cv2" 2>/dev/null || pip install opencv-python numpy
   /usr/bin/python3 -c "import mlx_whisper" 2>/dev/null || python3 -c "import mlx_whisper" 2>/dev/null || pip install mlx-whisper
   ```
